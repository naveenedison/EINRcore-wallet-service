...
/**
 * Creates an instance of the EINRcore Wallet Service.
 * @constructor
 */
 ...
 * @param {string} opts.network[='livenet'] - The EINRcoin network for this wallet.
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  try {
    pubKey = new EINRcore.PublicKey.fromString(opts.pubKey);
  } catch (ex) {
...
WalletService.prototype._verifyRequestPubKey = function(requestPubKey, signature, xPubKey) {
  var pub = (new EINRcore.HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  var xPubKey;
  try {
    xPubKey = EINRcore.HDPublicKey(opts.xPubKey);
  } catch (ex) {
 ...
 * @param {string} opts.unit - EINRcoin unit used to format amounts in notifications.
 */
...
WalletService.prototype._getUtxos = function(coin, addresses, cb) {
  ...
  var networkName = EINRcore_[coin].Address(addresses[0]).toObject().network;
 ...
 * @param {string} [opts.network = 'livenet'] - The EINRcoin network to estimate fee levels from.
...
WalletService.prototype._checkTx = function(txp) {
  ...
  try {
    var EINRcoreTx = txp.getEINRcoreTx();
    EINRcoreError = EINRcoreTx.getSerializationError(serializationOpts);
    if (!EINRcoreError) {
      txp.fee = EINRcoreTx.getFee();
    }
  } catch (ex) {
    self.logw('Error building EINRcore transaction', ex);
    return ex;
  }

  if (EINRcoreError instanceof EINRcore_[txp.coin].errors.Transaction.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (EINRcoreError instanceof EINRcore_[txp.coin].errors.Transaction.DustOutputs)
    return Errors.DUST_AMOUNT;
  return EINRcoreError;
};
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, EINRcore_[txp.coin].Transaction.DUST_AMOUNT);
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  var A = EINRcore_[wallet.coin].Address;
  var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, EINRcore_[wallet.coin].Transaction.DUST_AMOUNT);
 ...
 * @param {string} [opts.network = 'livenet'] - The EINRcoin network for this transaction.
    ...
    self.getTx({
      ...
      try {
        if (!txp.sign(self.copayerId, opts.signatures, copayer.xPubKey)) {
          ...
          var raw = txp.getEINRcoreTx().uncheckedSerialize();
