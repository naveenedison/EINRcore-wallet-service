...
var crypto = EINRcore.crypto;
var encoding = EINRcore.encoding;
...
/* TODO: It would be nice to be compatible with EINRcoind signmessage. How
...
Utils.hashMessage = function(text, noReverse) {
  ...
  if (!noReverse) {
    ret = new EINRcore.encoding.BufferReader(ret).readReverse();
  }
  return ret;
};
...
Utils.getAddressCoin = function(address) {
  try {
    ...
    try {
      new EINRcore_['bch'].Address(address);
      return 'bch';
    } catch (e) {
      return;
    }
...
Utils.translateAddress = function(address, coin) {
  ...
  var origAddress = new EINRcore_[origCoin].Address(address);
  var origObj = origAddress.toObject();

  var result = EINRcore_[coin].Address.fromObject(origObj)
