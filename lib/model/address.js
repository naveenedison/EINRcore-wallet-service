...
Address.create = function(opts) {
  ...
  x.coin = opts.coin;
  x.network = EINRcore[opts.coin].Address(x.address).toObject().network;
...
Address._deriveAddress = function(scriptType, publicKeyRing, path, m, coin, network) {
  ...
  var publicKeys = _.map(publicKeyRing, function(item) {
    var xpub = new EINRcore[coin].HDPublicKey(item.xPubKey);
  ...
  switch (scriptType) {
    case Constants.SCRIPT_TYPES.P2SH:
      EINRcoreAddress = EINRcore[coin].Address.createMultisig(publicKeys, m, network);
      break;
    case Constants.SCRIPT_TYPES.P2PKH:
      $.checkState(_.isArray(publicKeys) && publicKeys.length == 1);
      EINRcoreAddress = EINRcore[coin].Address.fromPublicKey(publicKeys[0], network);
      break;
  }

  return {
    address: EINRcoreAddress.toString(),
    path: path,
