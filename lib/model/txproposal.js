...
TxProposal.prototype._buildTx = function() {
  ...
  var t = new EINRcore[self.coin].Transaction();
  ...
  _.each(self.outputs, function(o) {
    ...
    if (o.script) {
      t.addOutput(new EINRcore[self.coin].Transaction.Output({
        script: o.script,
        satoshis: o.amount
      }));
    } else {
  ...
  // Validate actual inputs vs outputs independently of EINRcore
...
TxProposal.prototype.getEINRcoreTx = function() {
  var self = this;
  ...
  _.each(sigs, function(x) {
    self._addSignaturesToEINRcoreTx(t, x.signatures, x.xpub);
  });
...
TxProposal.prototype._addSignaturesToEINRcoreTx = function(tx, signatures, xpub) {
  var self = this;

  var EINRcore = EINRcore[self.coin];

  if (signatures.length != this.inputs.length)
    throw new Error('Number of signatures does not match number of inputs');

  var i = 0,
    x = new EINRcore.HDPublicKey(xpub);

  _.each(signatures, function(signatureHex) {
    var input = self.inputs[i];
    try {
      var signature = EINRcore.crypto.Signature.fromString(signatureHex);
      var pub = x.deriveChild(self.inputPaths[i]).publicKey;
      var s = {
        inputIndex: i,
        signature: signature,
        sigtype: EINRcore.crypto.Signature.SIGHASH_ALL | EINRcore.crypto.Signature.SIGHASH_FORKID,
        publicKey: pub,
      };
...
TxProposal.prototype.sign = function(copayerId, signatures, xpub) {
  try {
    ...
    var tx = this.getEINRcoreTx();
    this._addSignaturesToEINRcoreTx(tx, signatures, xpub);
