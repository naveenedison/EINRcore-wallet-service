...
helpers.signMessage = function(text, privKey) {
  var priv = new EINRcore.PrivateKey(privKey);
  var hash = Utils.hashMessage(text);
  return EINRcore.crypto.ECDSA.sign(hash, priv, 'little').toString();
};

helpers.signRequestPubKey = function(requestPubKey, xPrivKey) {
  var priv = new EINRcore.HDPrivateKey(xPrivKey).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).privateKey;
...
helpers._generateCopayersTestData = function() {
  ...
  _.each(xPrivKeys, function(xPrivKeyStr, c) {
    var xpriv = EINRcore.HDPrivateKey(xPrivKeyStr);
    var xpub = EINRcore.HDPublicKey(xpriv);

    var xpriv_45H = xpriv.deriveChild(45, true);
    var xpub_45H = EINRcore.HDPublicKey(xpriv_45H);
    ...
    var xpub_44H_0H_0H = EINRcore.HDPublicKey(xpriv_44H_0H_0H);
    ...
    var xpub_1H = EINRcore.HDPublicKey(xpriv_1H);
...
helpers.randomTXID = function() {
  return EINRcore.crypto.Hash.sha256(new Buffer(Math.random() * 100000)).toString('hex');;
};
...
helpers.stubUtxos = function(server, wallet, amounts, opts, cb) {
  ...
  var S = EINRcore_[wallet.coin].Script;
...
helpers.clientSign = function(txp, derivedXPrivKey) {
  ...
  var xpriv = new EINRcore.HDPrivateKey(derivedXPrivKey, txp.network);
